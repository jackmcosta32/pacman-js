...
- `src/app/pacman/scenes`: scene instances for the match and menu.
...
3. `PacmanGame.start()` selects the current scene and calls `scene.init()`.
...
- Scene loading is currently static and marked for future dynamic loading.
